...
function verifyWorker (signer, report, reportCa, reportCert, reportSig) {
    const pki = forge.pki;
    var cert = pki.certificateFromPem (reportCert);

    var md = forge.md.sha256.create ();
    md.update (report, 'utf8');
    var signature = Buffer.from(web3Utils.hexToBytes (reportSig));


    // verify data with a public key
    // (defaults to RSASSA PKCS#1 v1.5)
    var verified = cert.publicKey.verify (md.digest ().bytes (), signature);
    if (!verified) {
